        ...
        if (discoveryDocument.hasOwnProperty('jwks_uri')) {
          ...
          axios.get(discoveryDocument.jwks_uri)
            ...
            .catch(function(error) {
              ...
              internalServerError("Internal server error 5: " + error.message,callback);
            });
        } else {
          console.log("Internal server error: Unable to find JWK in discovery document");
          internalServerError("Internal server error 4: Unable to find JWK in discovery document",callback);
        }
      })
      .catch(function(error) {
        console.log("Internal server error: " + error.message);
        internalServerError("Internal server error 3: " + error.message,callback);
      });
  } else {
    mainProcess(event, context, callback);
  }
...
function mainProcess(event, context, callback) {
  ...
  if (request.uri.startsWith(config.CALLBACK_PATH)) {
    ...
    axios.post(discoveryDocument.token_endpoint, postData)
      .then(function(response) {
        ...
        try {
                ...
                const response = {
                  ...
                  "headers": {
                    ...
                    "set-cookie" : [
                      ...
                      {
                        "key": "Set-Cookie",
                        "value" : cookie.serialize('email', decodedData.payload.email, {
                          path: '/',
                          maxAge: config.SESSION_DURATION
                        }).concat("; SameSite=None").concat("; Secure")
                      }
          ...
          console.log("Internal server error: " + error.message);
          internalServerError("Internal server error 2: " + error.message,callback);
        }
      })
      .catch(function(error) {
        console.log("Internal server error: " + error.message);
        internalServerError("Internal server error 1: " + error.message,callback);
      });
  } 
  else if(request.uri.startsWith("/Functions/main.html") || request.uri.startsWith("/Functions/Master.js") || request.uri.startsWith("/Scripts/")  || request.uri.startsWith("/Functions/Taskpane") || request.uri.startsWith("/Functions/External_CSS/")){
    console.log("Requesting main.html ;  master.js ; Script folder ; Taskpane.html");
    ...
    // runSearchInOrder(request, headers, callback);
    // fun
    // sendMain(request, headers, callback);
    // console.log("Redirecting to OIDC provider.");
    redirect(request, headers, callback);
  }
}

function runSearchInOrder(request, headers,callback) {
    sendMain(function() {
            redirect(callback);
...
function sendMain(callback) {
  ...
  const response = {
    ...
    "headers": {
      "location" : [{
        "key": "Location",
        "value": "https://d1xbl4boloxruq.cloudfront.net/Functions/main.html"
      }]
    },
  };
  callback(null, response);
      ...
      "set-cookie" : [
        ...
        },
        {
          ...
          "value" : cookie.serialize('NONCE', n[1], {
            ...
            // secure:true,
            // httpOnly: true
          }).concat("; SameSite=None").concat("; Secure").concat("; HttpOnly")
        }
      ],
    },
  };
...
function internalServerError(error_description,callback) {
  let page = `
  <!DOCTYPE html>
  <html lang="en">
  <head>
  ...
  <body>
      <div class="cover"><h1>Internal Server Error <small>Error 500</small></h1>
